...
<div class="tab" >
  ...
  <a href="{{ route('user.account.payable_booking') }}">
    <button class="tablinks active" onclick="openCity(event, 'approved')"> Booking </button>
  </a>
        ...
        <tbody>
            ...
            <?php $check = true;
             if( $booking->status == 'partial_paid') {
                if(account_cheque::find($booking->cheque_id)!= null){
                    if(account_cheque::find($booking->cheque_id)->status == 'not_cleared'){
                        $check = true;
                    }else{
                        $check = false;
            ...
            ?>
            @if($check == true)
            <tr style="background-color:  <?php if(account_cheque::find($booking->cheque_id)!= null) if(account_cheque::find($booking->cheque_id)->status == 'not_cleared'){ ?> gold; <?php } ?>">
                
                <td><span class="badge badge-pill ">{{ $booking->id }}</span> </td>
                <td><span class="badge badge-pill ">{{ $booking->booking_id }}</span></td>
                <td><span class="badge badge-pill ">{{ $booking->type }}</span></td>

                <td><span class="badge badge-pill ">{{ $booking->total_amount }}</span></td>
                <td><span class="badge badge-pill ">{{ $booking->amount_remaning }}</span></td>
                <td><span class="badge badge-pill ">{{ $booking->amount_paid }}</span></td>
                <td><span class="badge badge-pill badge-warning">{{ $booking->booking_date }}</span></td>

                <td>
                    @if($booking->amount_remaning > 0 && $booking->amount_paid > 0)
                        partial_paid
                    @endif
                    @if($booking->amount_remaning == 0 &&   $booking->amount_paid == $booking->total_amount )
                    paid
                    @endif
                ...
                <td>
                    @if( $booking->pay_by == 'cheque')
                      @if( $booking->pay_by == 'cheque' && $booking->amount_paid > 0)
                      <form action="{{ route( 'user.account.view_cheque') }}" method="post" class="d-inline">
                          @csrf
                          <input type="text" class="form-control d-none" name="id" value ="{{ $booking->cheque_id }}" placeholder="Enter id" >
                          <button type="submit" id="{{ $booking->cheque_id }}" class="btn btn-success edit_cheque" data-toggle="" data-target="">
                              View Cheque
                          </button>
                      </form>
                      @else
                      <button type="submit" id="{{ $booking->id }}" class="btn btn-primary edit_cheque" data-toggle="modal" data-target="#exampleModal" style=" background-color: #08c;">
                          Issue Cheque
                      </button>
                      @endif
                    @else
                        <select name="pay_by" id="{{ $booking->id }}" class="form-control pay_by">
                ...
                <!-- <td>
                    ...
                    <form action="{{ route( 'user.booking.view_booking') }}" method="post" class="d-inline">
                        @csrf
                        <input type="text" class="form-control d-none" name="id" value ="{{$booking->po_id}}" placeholder="Enter id" >
...
<div class="modal fade  " id="exampleModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{ route('user.account.cheque_issue_booking') }}" enctype="multipart/form-data" method="post">
          @csrf
      ...
      $(".pay_by").on('change', function(){
          ...
          if(this.value == 'cheque'){
            ...
            swal({
                ...
                $.ajax({
                    type:'POST',
                    url:"{{ route('user.account.pay_by_petty_booking') }}",

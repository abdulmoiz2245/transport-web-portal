...
<div class="sk-circle loader"style="display:none">
  ...
  <div class="sk-circle12 sk-child"></div>user


</div>

<div class="tab" >
  <button  class="tablinks  "> <a href="{{ route('user.hr_pro.add_employee_termination') }}" style=""> Add New </a>  </button>
  <button class="tablinks active" onclick="openCity(event, 'terminated')"> Terminated </button>
...
<div class="card">
    <div class="card-body">
        ...
        <div class="mt-3 mb-3"> 
                ...
                <a href="{{ route( 'user.hr_pro.employee_termination_history') }}"target="_blank" class="ml-3">
                        <img src="<?= asset('assets') ?>/images/history_icon.png" alt="" width="30">
                </a> 

                <a href="{{ route( 'user.hr_pro.trash_employee_termination') }}" target="_blank" class="ml-3">
          ...
          <div class="table-responsive">
            <table id="trade_license"  class="display table  nowrap " style="width:100%">
                ...
                <tbody>
                    ...
                    @if($terminate_employee->status == 'approved'  && $terminate_employee->row_status != 'deleted')
                    @foreach($data['employees'] as $employees)
                    @if($employees->id ==  $terminate_employee->emp_id && $employees->row_status != 'deleted')
                    ...
                    <td>
                      <form action="{{ route( 'user.hr_pro.view_employee_termination') }}" method="post" class="d-inline">
                            @csrf
                            <input type="text" class="form-control d-none" name="id" value ="{{$terminate_employee->id}}" placeholder="Enter id" >
                            <button type="submit" class="border-0 .bg-white">
                                <img src="<?= asset('assets') ?>/images/eye_icon.png" alt="" width="34">
                            </button>
                        </form>
                        <a href="#" id="{{ $terminate_employee->id }}" class="delete-file">
        ...
        <div id="pending" class="tabcontent" >
          <div class="table-responsive">
            <table id="trade_license"  class="display table  nowrap " style="width:100%">
                ...
                <tbody>
                @foreach($data['terminate_employee'] as $terminate_employee)
                    @if($terminate_employee->status == 'pending'  && $terminate_employee->row_status != 'deleted')
                    @foreach($data['employees'] as $employees)
                    ...
                    <td>
                        ...
                        <form action="{{ route( 'user.hr_pro.edit_employee_termination') }}" method="post" class="d-inline">
                            @csrf
                            <input type="text" class="form-control d-none" name="id" value ="{{$terminate_employee->id}}" placeholder="Enter id" >
                            <button type="submit" class="border-0 .bg-white">
        ...
        <div id="rejected" class="tabcontent" >
          <div class="table-responsive">
            <table id="trade_license"  class="display table  nowrap " style="width:100%">
                ...
                <tbody>
                    @foreach($data['terminate_employee'] as $terminate_employee)
                    @if($terminate_employee->status == 'rejected'  && $terminate_employee->row_status != 'deleted')
                    @foreach($data['employees'] as $employees)
                    @if($employees->id ==  $terminate_employee->emp_id)
...
$('.delete-file').click(function () {
            ...
            swal({
                ...
                $.ajax({
                    type:'POST',
                    url:"{{ route( 'user.hr_pro.delete_employee_termination_status') }}",

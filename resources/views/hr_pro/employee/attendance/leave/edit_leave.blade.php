<div class="container">
    ...
    <div class="mb-5"> 
        <a href="{{ route( 'user.hr_pro.employee_leave') }}">
            <img  src="<?= asset('assets') ?>/images/back-button.png" alt="" width="30">
        </a>
    </div>
    <form action="{{ route('user.hr_pro.update_employee_leave') }}" method="post" enctype="multipart/form-data">
    @csrf
        ...
        <div class="row">
            <div class="col-md-6 col-12">
                <div class="form-group">
                    ...
                    <textarea name="status_message" class="form-control form-control-rounded"  placeholder="Enter Admin Notes">{{ $data['leave']->status_message }}</textarea>
            ...
            <div class="col-md-6 col-12">
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        <option value='pending' <?php if($data['leave']->status == 'pending') echo 'selected="selected"' ?> >Pending</option>
                        <option value='approved' <?php if($data['leave']->status == 'approved') echo 'selected="selected"' ?> >Approved</option>
                        <option value='rejected' <?php if($data['leave']->status == 'rejected') echo 'selected="selected"' ?>>Rejected</option>
                    </select>
                </div>
            </div>
        </div>
